...
function App() {
  ...
  useEffect(() => {
    const listener = listen<SystemInfo>("update_system_info", (e) => setSystemInfo(e.payload));
  ...
  return (
    <div>
      ...
      <select onChange={updateFont}>
        {fonts.map((f) => <option selected={f === font}>{f}</option>)}
      </select>
      <table>
        ...
        {systemInfo.processes.map((process) => <tr>
          <td>{process.pid}</td>
          <td>{process.name}</td>
          <td>{process.cpu}</td>
